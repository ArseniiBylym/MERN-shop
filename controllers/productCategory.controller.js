...
exports.addProductCategory = async (req, res, next) => {
    ...
    try {
        if (!subCategories || subCategories.length === 0) {
            ...
            return res.status(201).json({msg: `New category was successfully added`, categoryItem: savedCategory});
        }
        ...
        return res.status(200).json({msg: `New subCategories was successfully added`, categoryItem: updatedCategory});
    } catch (error) {
        next(error);
    }
};
...
exports.deleteProductCategory = async (req, res, next) => {
    ...
    try {
        if (!subCategoryName) {
            ...
            res.status(200).json({msg: `Category was successfully deleted`, categoryId});
        }
        ...
        await category.save();
        return res.status(200).json({msg: `SubCategory was successfully deleted`, subCategoryName});
    } catch (error) {
        next(error);
    }
};
